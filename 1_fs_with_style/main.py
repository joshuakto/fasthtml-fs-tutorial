...
def render_file_list(tree: List[Tuple[str, str, str]], current_path: str) -> Div:
    return Div(cls="flex flex-col h-full")(
        # Fixed header
        Div(cls="bg-gray-50 sticky top-0 z-10")(
            Div(cls="flex text-left text-xs font-medium text-gray-500 uppercase tracking-wider")(
                Div("Name", cls="w-2/5 p-3"),
                Div("Size", cls="w-1/6 p-3 text-right"),
                Div("Kind", cls="w-1/6 p-3 text-center"),
                Div("Date Added", cls="w-1/4 p-3 text-right"),
            )
        ),
        # Scrollable content
        Div(cls="flex-1 overflow-auto")(
            Div(cls="divide-y divide-gray-200")(
                *[Div(cls="flex hover:bg-gray-50")(
                    Div(cls="w-2/5 p-3 flex items-center space-x-2")(
                    ...
                    Div(format_size(get_file_info(item[2])[0]), cls='w-1/6 p-3 text-right text-gray-500 text-sm'),
                    Div(Div(get_file_info(item[2])[2], cls='truncate'), cls='w-1/6 p-3 text-left text-gray-500 text-sm'),
                    Div(Div(format_date(get_file_info(item[2])[1]), cls='truncate'), cls='w-1/4 p-3 text-right text-gray-500 text-sm'),
                ) for item in tree]
            )
        )
    )
        ...
        Div(cls="w-64 bg-white shadow-lg fixed h-full")(
            Div(cls="p-4")(
                Input(type="text", cls="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500",
                    ...
                    hx_get=f"/{path}", hx_trigger="keyup changed delay:200ms", 
